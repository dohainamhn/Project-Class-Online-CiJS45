    ...
    switch (screen) {
                ...
                if (checkNull && checkChannelName) {
                    fetch(url, requestInit)
                        ...
                        .then(function (json) {
                            ...
                            console.log(json);
                            const data = {
                                channel: controller.removeVietnameseTones(
                                    createRoomForm.chanelName.value
                                ),
        ...
        case "classRoomScreen": {
            ...
            roomInfo.host == firebase.auth().currentUser.email
                ? agora.joinChannel(roomInfo.channel, true)
                : agora.joinChannel(roomInfo.channel, false);
            ...
            signOutBnt.addEventListener("click", () => {
                ...
                model.removeUserInRoom(
                    agora.localStreams.camera.id,
                    model.currentRoomID
                );
        ...
        case "updatePageScreen": {
            ...
            searchBar.addEventListener("keyup", (e) => {
                ...
                const filteredCharacters = roomSearch.filter((character) => {
                    return character.email.toLowerCase().includes(searchString);
                });
                for (let index = 0; index < roomSearch.length; index++) {
                    if (filteredCharacters[index] !== undefined)
...
view.chat = async () => {
    ...
    inputChatEmail.addEventListener("keyup", async (e) => {
        if (e.keyCode == "13") {
            ...
            if (
                inputChatEmail.value == firebase.auth().currentUser.email ||
                friend == null
            ) {
                alert("Email is invalid ");
                return;
            }
            let data = await model.findConversation(
                "conversations",
                "users",
            ...
            if (data == undefined) {
                ...
                let data2 = await model.findConversation(
                    "conversations",
                    "users",
                    inputChatEmail.value
                );
                ...
                messageBox.scrollTop = messageBox.scrollHeight;
                let allconversation = await model.getDataFireStore(
                    "conversations",
                    "users",
                    "array-contains"
                );
                ...
                if (allconversation.length !== 0) {
                    for (let x of allconversation) {
                        conversations.push({
                            createdAt: controller.convertToTimeStamp(
                                x.data().messages[x.data().messages.length - 1]["createdAt"]
                            ),
                            messages: x.data().messages,
                            id: x.id,
                            users: x.data().users,
...
`;
    document
        .querySelector(".right-container .room-list")
        .appendChild(roomWrapper);
    ...
    deleteRoomBtn.addEventListener("click", () => {
        ...
        if (firebase.auth().currentUser.email == roomData.host) {
            document.querySelector(
                `#delete${roomID} .popup-form .title-popup`
            ).innerHTML = "Do you really want to delete this room?";
            document.querySelector(
                `#delete${roomID} .popup-form .button-popup`
            ).innerHTML = `
...
view.getRooms = (data) => {
    ...
    roomWrapper.innerHTML = `
    <div class="a" id="delete${data.id}">
            <i class="fas fa-trash-alt"></i>
            <div class="popup-form" id="popup-form${data.id}">
                <div class="title-popup"></div>
                <div class="button-popup">
    ...
    <div class="room-bar cursor" id="join-room-${data.id}">
...
`;
    ...
    let deleteRoomBtn = document.getElementById(`delete${data.id}`);
    let joinRoom = document.getElementById(`join-room-${data.id}`);
    joinRoom.addEventListener("click", async () => {
        if (data.password !== "") {
            var person = prompt("Please enter password");
    ...
    deleteRoomBtn.addEventListener("click", () => {
        ...
        if (firebase.auth().currentUser.email == data.host) {
            document.querySelector(
                `#delete${data.id} .popup-form .title-popup`
            ).innerHTML = "Do you really want to delete this room?";
            document.querySelector(
                `#delete${data.id} .popup-form .button-popup`
            ).innerHTML = `
             <button class="popup-bnt" id="yes${data.id}">Yes</button>
             <button class="popup-bnt" id="no${data.id}">No</button>
            `;
            document.getElementById(`yes${data.id}`).addEventListener("click", () => {
                model.deleteDataFireStore("rooms", data.id);
                document.getElementById(`${data.id}`).remove();
                console.log(`xoa room ID ${data.id}`);
            });
            document
                .getElementById(`no${data.id}`)
                .addEventListener("click", () => { });
        } else {
            document.querySelector(
                `#delete${data.id} .popup-form .title-popup`
        ...
        let arr = document.querySelectorAll(".popup-form");
        arr.forEach((item) => {
            if (item.id !== `popup-form${data.id}`) item.classList = "popup-form";
        });
        popup.classList.toggle("show-popup");
    });
};
...
`;
    ...
    deleteRoomBtn.addEventListener("click", () => {
        ...
        if (firebase.auth().currentUser.email == room.host) {
            document.querySelector(
                `#delete${room.id} .popup-form .title-popup`
            ).innerHTML = "Do you really want to delete this room?";
            document.querySelector(
                `#delete${room.id} .popup-form .button-popup`
            ).innerHTML = `
...
view.getInFoRoom = async (roomID, room) => {
    ...
    let count = 0;
    realTimeUserInfo !== null
        ? (count = Object.keys(realTimeUserInfo).length)
        : (count = 0);
...
view.setProfileDefault = async () => {
    ...
    data.isTeacher
        ? (isTeacher.innerHTML = "Job: Teacher")
        : (isTeacher.innerHTML = "Job: Student");
    data.workAt == undefined
        ? (workAt.innerHTML = `Work at: `)
        : (workAt.innerHTML = `Work at:  ${data.workAt}`);
...
view.setEventListenEditProfile = () => {
    ...
    updateForm.addEventListener("submit", (e) => {
        e.preventDefault();
        let isTeacher = false;
        updateForm.isTeacher.checked == updateForm.isStudent.checked
            ? (isTeacher = "")
            : (isTeacher = updateForm.isTeacher.checked);
        const data = {};
        if (updateForm.workAt.value !== "") data.workAt = updateForm.workAt.value;
        if (isTeacher !== "") data.isTeacher = isTeacher;
        if (updateForm.aboutMe.value !== "")
            data.aboutMe = updateForm.aboutMe.value;
...
view.listenOnUpdateImage = () => {
    ...
    uploadImg.addEventListener("change", async (e) => {
        ...
        let imgName = e.target.value.slice(
            e.target.value.lastIndexOf("th") + 3,
...
view.adddDevidePageBtn = () => {
    ...
    for (let i = 1; i <= Math.ceil(model.rooms.length / 8); i++) {
        pageBnt += ` <button class="number${i}"  onclick = "nextBnt(${i})" class="nextButton">
        ${i}
        </button>`;
        view.allPages++ 
    }
