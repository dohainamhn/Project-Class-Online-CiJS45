...
controller.login = function (data) {
  if (data.email !== "" && data.password.value !== "") {
    model.login(data);
  }
};
controller.getDate = () => {
...
controller.checkUndefine = (item) => {
  if (
    item.data().messages[item.data().messages.length - 1]["content"] !==
    undefined
  ) {
...
controller.checkIconChat = (data) => {
  let find = icon.filter((item) => data.includes(item.syntax));
  console.log(find);
  if (find !== undefined) return find;
  else return null;
};
controller.returnSyntaxIconChat = (input) => {
  let inputMessage = document.querySelector(".input-message input");
  let find = icon.find((item, index) => index == input);
  inputMessage.value += find.syntax;
};
